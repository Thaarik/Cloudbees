...
const UserPage = async ({ username }) => {
  ...
  // to retreive idividual user details
  const gitHubUser = gitHubUsersDetail.userdata.find(
    (user) => username === user.login
  );
  const user = gitHubUser.userDetail;
      ...
      <div className="flex flex-col w-4/6 m-4 justify-start items-center border-2 border-slate-500 p-4 rounded-lg cursor-default text-slate-800 dark:text-slate-200">
        {user.name && (
          <div className="m-2 text-xl mobile:text-base cursor-default">
            <span className=" font-extrabold  mobile:font-bold">Name: </span>
            {user.name}
          </div>
        )}
        {user.login && (
          <div className="m-2">
            <span className="font-extrabold mobile:font-bold ">Username: </span>
            {user.login}
          </div>
        )}
        {user.bio && (
        ...
        {user.location && (
          <div className="m-2">
            <span className="font-extrabold mobile:font-bold">Location: </span>
            {user.location ? user.location : "None"}
          </div>
        )}
        {user.company && (
          <div className="m-2">
            <span className="font-extrabold mobile:font-bold">Company: </span>
      ...
      <details className="w-5/6 bg-slate-300 rounded-lg group mb-3 transition ease-in text-slate-900 dark:text-slate-200 dark:bg-slate-800">
        ...
        <div className="p-4 flex flex-wrap h-auto justify-evenly items-center">
          ...
          <div className="flex flex-col m-3">
            ...
            <div className="m-2">
              <span className="font-extrabold">Number of Public Gists:</span>{" "}
              {user.public_gists}
            </div>
            {user.hireable !== null && (
              <div className="m-2">
